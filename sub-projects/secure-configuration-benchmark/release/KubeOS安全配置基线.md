...
```bash
执行vlock -v命令，若存在返回值则表示软件包存在，反之软件包未安装
```
...
要求使用独立于信息系统的认证设备才能接入的多因素解决方案包括：提供硬件令牌以及智能卡

特权账户是指具有特权用户授权权限的信息系统账户。

远程访问是指授权用户（或信息系统）通过外部、非组织控制的网络，访问非公开信息系统的行为。远程访问方式包括拨号、宽带和无线接入等。

本要求仅适用于具备特定设备功能概念或存在组织用户概念的组件（例如 VPN、代理功能），不适用于为配置设备本身（即管理目的）而进行的身份认证。
...
需要在镜像制作时安装pam_pkcs11、nss、nss-util、ccid、 pcsc-lite、pcsc-tools、opensc
...
```bash
修改/etc/sysctl.conf 文件，将kernel.dmesg_restrict配置值为1， 并执行sysctl -p /etc/sysctl.conf，使其永久生效：
# vim /etc/sysctl.conf
kernel.dmesg_restrict = 1
```
...
### 3.13 KubeOS默认情况下不得转发IPv6源路由数据包:
...
### 4.1 除非确实有使用kdump服务的必要，否则必须禁用kdump服务。
...
内核的core dump可能包含系统在崩溃时的全部内存内容，core dump可能会占用大量磁盘空间，并可能导致拒绝服务（DoS）攻击，从而耗尽目标文件系统分区上的可用空间。
...
### 5.5 通过网络文件系统（NFS）导入的 KubeOS 文件系统，必须挂载 noexe 以防止执行二进制文件。
...
### 5.7 包含用户/home目录的 KubeOS 文件系统，必须挂载setuid 和 setgid位以防止执行文件。
...
# find -L  /usr/local/bin /usr/local/sbin -perm /022 -type d -exec chmod 755 '{}' \;
# find -L  /bin /sbin /usr/bin /usr/sbin -perm /022 -type d -exec chmod 755 '{}' \;
# exit
# reboot
```
...
```bash
使用以下命令，验证系统命令是否具有“755”模式或更宽松的权限，若无返回值，则pass，否则fail：
...
```bash
使用以下命令，验证系统范围内共享库目录“/lib”、“/lib64”、“/usr/lib”和“/usr/lib64”是否具有“755”模式或更宽松的权限，若无返回值，则pass，否则fail：
# find -L /lib /lib64 /usr/lib /usr/lib64 -perm /022 -type d -exec stat -c "%n %a" '{}' \;
```
...
# find -L /lib /lib64 /usr/lib /usr/lib64 -type f -name '*.so*' -perm /022 -exec stat -c "%n %a" {} +
```
...
```bash
配置系统范围内共享库文件，其权限模式应为0755或更低权限。可使用以下命令进行设置：
# find -L /lib /lib64 /usr/lib /usr/lib64 -type f -name '*.so*' -perm /022 -exec chmod go-w {} +
# exit
# reboot
```

### 6.5 所有 KubeOS 本地交互式用户的home目录（Home Directories）权限必须设置为 750 或更低（即权限值不大于 750）。
...
本地交互式用户home目录若权限设置过大，可能导致其他用户未经授权使用（Access）和访问（Access）该用户的文件。
...
```bash
使用以下命令，验证系统中本地交互式用户所分配的主目录是否具有“750”或更宽松的权限，如果 "/etc/passwd" 中引用的主目录不具有“750”或更严格的权限，则fail：
...
# chmod 640 /etc/ssh/ssh_host*key
```
...
# find -L /lib /lib64 /usr/lib /usr/lib64 -type f -name '*.so*' ! -user root -exec stat -c "%n %U" {} +
```
...
# find -L /lib /lib64 /usr/lib /usr/lib64 -type f -name '*.so*' ! -user root -exec chown root {} +
# exit
# reboot
```
...
```bash
使用以下命令检查，若无返回值，则pass，否则fail：
# find -L /lib /lib64 /usr/lib /usr/lib64 -type f -name '*.so*' ! -group root -exec stat -c "%n %G" {} +
```
...
# find -L /lib /lib64 /usr/lib /usr/lib64 -type f -name '*.so*' ! -group root -exec chown :root {} +
# exit
# reboot
```
...
```bash
使用以下命令检查，若无返回值，则pass，否则fail：
# find -L /bin /sbin /usr/bin /usr/sbin /usr/local/bin /usr/local/sbin ! -user root -type f -exec stat -c "%n %U" '{}' \;
```
...
如果用户被分配未拥有文件相同的用户标识符 (UID)，则可能会无意中继承未拥有文件。
...
```bash
使用以下命令检查，以xfs为例，若无返回值，则pass，否则fail：
...
如果没有有效组所有者的文件被分配没有有效组所有者的文件相同的组标识符 (GID)，则可能会无意中继承这些文件。
...
### 6.20 所有 KubeOS 全局可写目录必须由 root、sys、bin 或应用程序组所有。
...
如果一个可被所有人写入的目录设置了粘滞位，并且不属于特权组标识符（GID）拥有的组，未经授权的用户可能能够修改其他人创建的文件。

唯一授权的公共目录是那些系统提供的临时目录或专为临时文件存储设计的目录。此设置通常保留给系统使用的目录以及用户用于临时文件存储的目录（例如，/tmp），以及需要全局读/写访问的目录。
...
```bash
使用以下命令检查，若返回值的第3列和第4列均为root、sys、bin或应用程序所在组，则pass，否则fail：
...
```bash
将系统中所有具有全局可写权限的目录的所属组更改为root或sys、bin或应用程序所在组，可以参考以下命令：
# chgrp root <目录>
```

### 6.21 所有 KubeOS 可写目录必须设置粘滞位。
...
此要求通常适用于信息技术产品的设计，但也可能适用于配置特定使用此类产品的信息系统组件。这可以通过其他政府机构的接受/验证过程进行验证。
...
### 6.22 KubeOS 必须防止未经授权的用户访问系统错误日志。
...
只有授权人员才应知晓错误及其详细信息。错误日志是组织运营状态的指示器，或者可以识别 KubeOS 或平台。此外，不得通过错误日志向未经授权的人员或其指定代表泄露个人身份信息 (PII) 和运营信息。

组织和发展团队必须仔细考虑错误日志的结构和内容。信息系统识别和处理错误条件的程度由组织政策和运营需求指导。
...
```bash
使用以下命令检查，若返回值为/var/log/messages 属主为root:root，权限小于等于640，则pass，否则fail：
...
### 6.23 KubeOS 必须生成提供满足所需信息的错误日志，同时不要泄露可能被利用的信息。
...
组织仔细考虑错误消息的结构/内容。信息系统识别和处理错误条件的程度由组织政策和运营需求指导。可能被对手利用的信息例如错误登录尝试，其中误输入的密码作为用户名，可以从记录的信息中派生（如果未明确说明）的任务/业务信息，以及个人信息，例如账号、社会安全号码和信用卡号码。
...
```bash
执行stat -c "%n %U:%G %a" /home/{用户名}检查文件是否存在，权限是否正确
```
...
### 6.25 所有 KubeOS 本地初始化程序不应该具有全局可写权限。
...
如果用户启动文件执行全局可写程序，特别是在受保护目录中，它们可能被恶意修改以破坏用户文件或以其他方式在用户级别损害系统。如果系统在用户级别被执行，则更容易提升权限，最终通过root权限危害系统。
...
### 7.1 KubeOS 必须根据端口、协议类别分配列表 (CAL) 和漏洞评估，禁止或限制使用功能、端口、协议或服务。
...
此外，操作系统远程访问功能必须能够立即断开当前远程访问信息系统用户的连接和禁用进一步的远程访问。断开连接或禁用的速度根据任务功能的紧迫性以及消除对组织信息系统的即时或未来远程访问的需要而有所不同。
...
### 8.1 KubeOS 时钟必须连接联网系统，至少每 24 小时同步一次时间源。
...
不准确的时间戳使得关联事件更加困难，并可能导致不准确的分析。确定系统在特定事件发生时的正确时间对于进行取证分析和调查系统事件至关重要。超出配置的可接受允许值（漂移）之外的源可能不准确。

同步内部信息系统时钟为具有多个系统时钟和通过网络连接的系统提供统一的时间戳。
...
```bash
使用以下命令检查，若返回值为server <time_source> maxpoll 16，则pass，否则fail：
...
```bash
使用以下命令将接口的混杂模式设置为关闭：
# ip link set dev <设备名称> promisc off
```

## 10 openssh

### 10.1 KubeOS 必须配置为：所有与 SSH 流量关联的网络连接在无响应后能够终止。
...
在短时间内终止无响应的 SSH 会话，可减少未经授权的人员接管已在控制台或控制台端口上启用且无人看管的管理会话的机会窗口。此外，快速终止空闲的 SSH 会话也将释放由被管网络占用的资源。

终止与通信会话关联的网络连接包括，在操作系统级别分配关联的 TCP/IP 地址/端口对，以及在多个应用程序会话使用单个操作系统级别网络连接时，在应用程序级别分配网络资源。这并不意味着操作系统终止所有会话或网络访问，它仅结束无响应的会话并释放与该会话关联的资源。
...
远程访问是指授权用户（或信息系统）通过外部、非组织控制的网络通信，对非公开信息系统的访问。远程访问方法例如拨号、宽带和无线。
...
```bash
使用以下命令检查，若提示需要输入密码则pass，否则fail：
# ssh-keygen -y -f /root/.ssh/*key
注：需要检查目录下所有私钥文件
```
...
```bash
执行如下命令，若返回值中包含active/enabled，则pass，否则fail
...
### 10.12 KubeOS 必须禁止通过 SSH 进行无人值守或自动登录。
...
```bash
执行如下2条命令：若存在返回值则表示已限制认证用户的系统访问
...
```
修改/etc/ssh/sshd_config文件，配置以下字段为no，重启sshd服务：
PermitUserEnvironment no
PermitEmptyPasswords no
# systemctl restart sshd.service
```

### 10.13 KubeOS 必须实施加密机制以保护 SSH 远程连接的机密性。
...
```bash
修改/etc/ssh/sshd_config文件，增加配置，执行服务重启
# vim /etc/ssh/sshd_config
Ciphers aes256-ctr,aes192-ctr,aes128-ctr
# systemctl restart sshd.service
```

### 10.14 KubeOS SSH 守护进程必须配置为仅使用 FIPS 140-2/140-3 批准的密码哈希算法的消息认证码 (MACs)。
...
远程访问（例如 RDP）是指授权用户（或信息系统）通过外部、非组织控制的网络通信，对非公开信息系统的访问。远程访问方法例如拨号、宽带和无线。

用于保护信息完整性的密码机制例如使用非对称加密的签名哈希函数，使公共密钥能够验证哈希信息，同时保持用于生成哈希的秘密密钥的机密性。
...
```bash
执行如下命令，若存在返回值则表示服务器已经配置为仅使用FIPS验证的密钥交换算法
...
此要求适用于与 KubeOS 一起使用的外围无线技术（例如无线鼠标、键盘、显示器等）。无线外围设备（例如 Wi-Fi/蓝牙/红外键盘、鼠标、指点设备和近场通信 [NFC]）通过创建计算机上的开放、未受保护的端口带来独特的挑战。无线外围设备必须满足无线数据传输的要求，并经 AO 批准使用。尽管某些无线外围设备（例如鼠标和指点设备）通常不携带需要保护的信息，但与这些无线外围设备的通信修改可能被用于损害 KubeOS。位于受控边界物理保护之外的通信路径容易受到拦截和修改的可能性。
...
如果没有识别设备，系统会生成未标识或未知设备，从而导致恶意活动。
...
在“/etc/modprobe.d/50-blacklist.conf”文件中添加或修改以下行后，重启系统：
blacklist usb-storage
...
### 13.3 KubeOS shadow必须配置为在失败登录尝试后的登录提示之间强制执行至少五秒的延迟。
...
如果未为本地交互用户分配有效的主目录，则无法存储和控制他们应拥有的文件。
...
```bash
执行grep -i path= /home/<username>/.*bash_profile
...
注：<emergency_administrator_account_name>是管理员账户名，非实际账户
```
...
# chage -E -1 <emergency_administrator_account_name>
...
### 13.10 KubeOS必须在密码35天不活跃后禁用帐户标识符（个人、组、角色和设备）
...
KubeOS必须跟踪不活动的时间段，并在35天不使用后禁用应用程序标识符。
...
```
修改/etc/pam.d/login文件配置：
session required pam_lastlog.so showfailed
```

### 13.13 KubeOS必须在15分钟不活动后启动会话锁定
...
### 13.15 KubeOS必须强制在通过可插拔身份验证模块（PAM）登录尝试失败后,至少延迟五秒钟再显示登录提示
...
```bash
执行grep -w "^password requisite pam_pwquality.so" /etc/pam.d/common-password，若存在返回值，则pass，否则fail
```
...
密码复杂度，或强度，是衡量密码抵抗猜测和暴力攻击企图的有效性的指标。密码长度是几个因素中的一个，有助于确定密码的强度和破解密码所需的时间。在密码中使用更多的字符有助于指数性地增加破坏密码所需的时间或资源。
...
```bash
执行grep -w "^password requisite pam_pwquality.so minlen=15" /etc/pam.d/common-password，若存在返回值，则pass，否则fail
```
...
```bash
执行grep -w "^password requisite pam_pwquality.so difok=8" /etc/pam.d/common-password，若存在返回值，则pass，否则fail
```
...
### 13.30 KubeOS的用户密码必须使用至少24小时（一天）
...
若返回值中非系统用户<username>的值大于1，则pass，否则fail
注：<username>指可登录用户
```
...
```bash
执行passwd命令设置账户的口令修改间隔时间大于1天，示例为1天
# passwd -n 1 <username>
```
...
若返回值中非系统用户<username>的值小于60，则pass，否则fail
注：<username>指可登录用户
```
...
需要与信息系统分离的设备获得访问权限的多因素解决方案例如提供基于时间或挑战响应的验证器的硬件令牌和智能卡。
...
需要与信息系统分离的设备才能获得访问权限的多因素解决方案包括提供基于时间或挑战响应验证器的硬件令牌。
...
### 13.41 KubeOS必须配置为不覆盖可插拔身份验证模块（PAM）配置
...
### 13.46 KubeOS密码套件必须配置为使用足够数量的哈希轮数
...
如果没有对安全功能进行验证，安全功能可能无法正常运行，并且可能会忽略故障。安全功能定义为信息系统的硬件、软件或固件，负责实施系统安全策略并支持保护所基于的代码和数据的隔离。安全功能包括但不限于建立系统帐户、配置访问授权（即权限、特权）、设置要审计的事件以及设置入侵检测参数。

此要求适用于执行安全功能验证/测试的操作系统或需要此功能的系统和环境。
...
特权功能包括创建帐户、执行系统完整性检查或管理加密密钥等活动。非特权用户是指不具备相应授权的个人。例如特权功能能绕过入侵检测和防御机制或恶意代码防护机制，故需要采取措施防止非特权用户访问。
...
如果没有对安全功能进行验证，安全功能可能无法正确运行，并且可能会忽略故障。安全功能被定义为信息系统的硬件、软件或固件，负责实施系统安全策略并支持保护所基于的代码和数据的隔离。安全功能包括但不限于建立系统帐户、配置访问授权（即权限、特权）、设置要审计的事件以及设置入侵检测参数。
...
使用restorecon查找未刷新标签的目录、文件，并刷新：
查看标签是否正确：
# restorecon -Rvn /

刷新对应目录
# restorecon -Rv {taget directory}
# setenfore 1

注：若/proc/cmdline中selinux=0，则需要将grub.cfg中selinux=0配置为enforcing=0；
```
...
如果没有对安全功能进行验证，安全功能可能无法正确运行，并且故障可能会被忽视。安全功能定义为信息系统的硬件、软件或固件，负责实施系统安全策略并支持保护所基于的代码和数据的隔离。安全功能包括但不限于建立系统帐户、配置访问授权（即权限、特权）、设置要审计的事件以及设置入侵检测参数。

此要求适用于KubeOS执行安全功能验证/测试或需要此功能的系统和环境。
...
### 15.2 KubeOS文件完整性工具aide必须配置验证访问控制列表
...
检测此类更改并提供自动响应有助于避免可能最终影响KubeOS安全状态的意外的负面后果。KubeOS的信息系统安全管理员（ISSM）/信息系统安全官（ISSO）和系统管理员（SA）必须在未经授权的配置项修改时通过电子邮件或监控系统得到通知。
...
```bash
执行grep -R aide /etc/crontab /etc/cron.* | grep -v monthly，若有返回值，则pass，否则fail
```

**修复方法：**


修改/etc/cron.weekly/aide文件，添加或修改配置，此处以每周0点为例：
```bash
0 0 * * * aide --check | /bin/mail -s "$HOSTNAME - Daily AIDE integrity check run" root@example_server_name.mil
```
...
安全功能定义为信息系统的硬件、软件或固件，负责实施系统安全策略并支持保护所基于的代码和数据的隔离。安全功能包括但不限于建立系统帐户、配置访问授权（即权限、特权）、设置要审计的事件以及设置入侵检测参数。

信息系统提供的通知包括本地计算机控制台或硬件指示（例如指示灯）的消息。
...
```bash
修改/etc/cron.daily/aide文件，添加或修改配置，此处以每日0点为例：
...
满足此要求可能需要的审计记录内容包括时间戳、源地址和目标地址、用户/进程标识符、事件描述、成功/失败指示、涉及的文件名以及调用的访问控制或流控制规则。
...
### 16.5 当审计记录没有立即发送到中央审计记录存储设施时，KubeOS必须分配审计记录存储容量以存储至少一周的审计记录
...
为了确保KubeOS有足够的存储容量来写入审计日志，KubeOS必须能够分配审计记录存储容量。
...
当KubeOS面临无法按要求处理审核日志的风险时，它会采取措施来缓解故障。审核处理失败包括软件/硬件错误、审核捕获机制故障以及达到或超过审核存储容量。对审核失败的响应取决于失败模式的性质。
...
### 16.8 KubeOS 必须将审计记录保存到与被审计系统不同的系统或介质上
...
存储在一个位置的信息容易受到意外或偶然性的删除或篡改。远程存储是审计存储容量有限的信息系统中常见的处理流程。
...
```bash
使用grep命令查看配置：
# grep -i "^disk_full_action = syslog" /etc/audit/audisp-remote.conf
...
### 16.12 Audispd 必须将审计记录从被审计的 KubeOS 系统保存到不同的系统或介质上
...
执行grep remote_server /etc/audit/audisp-remote.conf若返回值中存在remote_server = <ip_address>，则pass，否则fail
...
### 16.13 信息系统安全官（ISSO）和系统管理员（SA）必须配置邮件别名，以便在 KubeOS 审计处理失败时接收通知
...
执行grep -i ""^root:"" /etc/aliases，如果 root 的别名未转发至受监控的电子邮件账户，或输出内容被注释掉，则fail
```

**修复方法：**

```bash
修改/etc/aliases文件，添加或修改配置，并执行newaliases：
# vim /etc/aliases
postmaster: root
root: box@server.mil
# newaliases
...
### 16.14 信息系统安全官（ISSO）和系统管理员（SA）必须在 KubeOS 审计处理故障事件发生时收到告警
...
-a always,exit -F path=/usr/bin/chacl -F perm=x -F auid>=1000 -F auid!=-1 -k prim_mod
重启auditd.service服务:
# service auditd restart
```
...
```bash
修改配置文件/etc/audit/rules.d/audit.rules，增加或修改：
-a always,exit -F path=/usr/bin/chcon -F perm=x -F auid>=1000 -F auid!=-1 -k prim_mod
重启auditd.service服务:
# service auditd restart
```
...
组织必须对特权命令进行全文记录审计。组织必须维护足够详细的审计跟踪，以重构事件并确定入侵的原因和影响。
...
```bash
修改配置文件/etc/audit/rules.d/audit.rules，增加或修改：
-a always,exit -F path=/usr/bin/chmod -F perm=x -F auid>=1000 -F auid!=-1 -k prim_mod
重启auditd.service服务:
# service auditd restart
```
...
```bash
修改配置文件/etc/audit/rules.d/audit.rules，增加或修改：
-a always,exit -F path=/usr/bin/chsh -F perm=x -F auid>=1000 -F auid!=-1 -k privileged-chsh
重启auditd.service服务:
# service auditd restart
```
...
通过如下命令修改配置：
# vim /etc/audit/rules.d/audit.rules
-a always,exit -F path=/usr/bin/newgrp -F perm=x -F auid>=1000 -F auid!=-1 -k privileged-newgrp
重启服务
# service auditd restart
```
...
通过如下命令修改配置：
# vim /etc/audit/rules.d/audit.rules
-a always,exit -F path=/sbin/pam_timestamp_check -F perm=x -F auid>=1000 -F auid!=-1 -k privileged-pam_timestamp_check
重启服务
# service auditd restart
```
...
通过如下命令修改配置：
# vim /etc/audit/rules.d/audit.rules
-a always,exit -F path=/usr/bin/rm -F perm=x -F auid>=1000 -F auid!=-1 -k prim_mod
重启服务
# service auditd restart
```
...
通过如下命令修改配置：
# vim /etc/audit/rules.d/audit.rules
-a always,exit -F path=/usr/bin/setfacl -F perm=x -F auid>=1000 -F auid!=-1 -k prim_mod
重启服务
# service auditd restart
```
...
通过如下命令修改配置：
# vim /etc/audit/rules.d/audit.rules
-a always,exit -F path=/usr/bin/ssh-agent -F perm=x -F auid>=1000 -F auid!=-1 -k privileged-ssh-agent
重启服务
# service auditd restart
```
...
通过如下命令修改配置：
# vim /etc/audit/rules.d/audit.rules
-a always,exit -F path=/usr/lib/ssh/ssh-keysign -F perm=x -F auid>=1000 -F auid!=-1 -k privileged-ssh-keysign
重启服务
# service auditd restart
```
...
通过如下命令修改配置：
# vim /etc/audit/rules.d/audit.rules
-a always,exit -F path=/usr/bin/su -F perm=x -F auid>=1000 -F auid!=-1 -k privileged-priv_change
重启服务
# service auditd restart
```
...
为了满足访问需求，KubeOS可以与满足或超过访问控制策略要求的企业级身份验证/访问/审计机制集成。
...
为了满足访问需求，KubeOS可以与满足或超过访问控制策略需求的企业级身份验证/访问/审计机制集成。
...
```bash
执行如下命令，如果返回值中的"delete_module" 系统调用的 "b32" 和 "b64" 审计规则均未定义，则fail，否则pass
...
通过如下命令修改配置：
# vim /etc/audit/rules.d/audit.rules
-a always,exit -F arch=b32 --a always,exit -F arch=b32 -S setxattr,fsetxattr,lsetxattr,removexattr,fremovexattr,lremovexattr -F auid>=1000 -F auid!=-1 -k perm_mod
...
```bash
执行如下命令，如果返回值中的"umount"系统调用没有定义所有 "b32" 和 "b64" 审计规则，则fail，否则pass
...
### 16.51 KubeOS必须为“unlink”、“unlinkat”、“rename”、“rmdir”系统调用的所有使用生成审计记录
...
### 16.54 KubeOS必须生成审计记录对“tallylog”文件的所有修改都必须生成审计记录
...
### 16.55 KubeOS在成功或未成功访问 "/etc/sudoers" 文件和 "/etc/sudoers.d/" 目录中的文件时生成记录
...
# grep -w "\-w /etc/sudoers " /etc/audit/rules.d/audit.rules
2.执行如下命令，若该命令没有返回任何输出，或者返回的行被注释掉，则fail，否则pass
# grep -w "\-w /etc/sudoers.d " /etc/audit/rules.d/audit.rules
```
...
通过如下命令修改配置：
# vim /etc/audit/rules.d/audit.rules
-a always,exit -F path=/usr/sbin/setfiles -F perm=x -F auid>=1000 -F auid!=-1 -k privileged-unix-update
重启服务
# service auditd restart
```
...
通过如下命令修改配置：
# vim /etc/audit/rules.d/audit.rules
-a always,exit -F path=/usr/sbin/semanage -F perm=x -F auid>=1000 -F auid!=-1 -k privileged-unix-update
重启服务
# service auditd restart
```

### 16.58 在KubeOS中使用“setsebool”必须生成审计记录。
...
至少，组织必须审计特权命令的全文记录。组织必须维护足够详细的审计跟踪，以重构事件，以确定危害的原因和影响。“setsebool”命令将特定SELinux布尔值或布尔值列表的当前状态设置为给定值。
...
通过如下命令修改配置：
# vim /etc/audit/rules.d/audit.rules
-a always,exit -F path=/usr/sbin/setsebool -F perm=x -F auid>=1000 -F auid!=-1 -k privileged-unix-update
重启服务
# service auditd restart
```

### 16.59 KubeOS必须为“/run/utmp”文件生成审计记录
...
默认情况下，KubeOS包含“-a task,never”审计规则作为默认的。此规则将禁止使用此规则启动的所有任务的系统调用审核。由于审计守护进程从上到下处理“audit.rules”文件，因此此规则取代了所有其他已定义的syscall规则；因此，操作系统上无法进行任何syscall审计。
...
### 16.63 KubeOS必须将所有帐户或帐户类型的并发会话数限制为10
...
如果没有对安全功能进行验证，安全功能可能无法正常运行，故障可能会被忽视。安全功能定义为信息系统的硬件、软件或固件，负责实施系统安全策略并支持保护所基于的代码和数据的隔离。安全功能包括但不限于建立系统帐户、配置访问授权（即权限、特权）、设置要审计的事件以及设置入侵检测参数。
...
```bash
在制作镜像时使用以下命令生成包含新密码的“grub.conf”文件，并替换：
# grub2-mkpasswd-pbkdf2
使用输出中的哈希值，修改“/etc/grub.d/40_custom”文件，并添加以下两行以在root条目中添加启动密码：
set superusers="root"
password_pbkdf2 root {输出中的哈希值}
...
```bash
在镜像制作时安装crypto-policies-scripts软件包
打开系统FIPS模式
